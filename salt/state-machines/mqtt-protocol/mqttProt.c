...
struct MQTTProt
{
    ...
    struct mqtt_client mqttc_client;
...
publish(MQTTProt *const me, RKH_EVT_T *pe)
{
    ...
    me->operRes = mqtt_publish(&me->mqttc_client,
                               me->config->topic,
                               appMsg.data,
                               appMsg.size,
...
recvFail(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    mqtt_recvFail(&realMe->mqttc_client, &localRecv); // an error occurred
}
...
parseRecv(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    memcpy(realMe->mqttc_client.recv_buffer.curr, evt->buf, evt->size);
    localRecv.rv = evt->size;
    mqtt_parseRecv(&realMe->mqttc_client, &localRecv);
}
...
sendMsgFail(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    mqtt_sendMsgFail(&realMe->mqttc_client, &localSend);
}
...
setMsgState(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_setMsgState(&realMe->mqttc_client, &localSend);
}
...
parseError(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_parseError(&realMe->mqttc_client, &localRecv);
}
...
noConsumed(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_noConsumed(&realMe->mqttc_client, &localRecv);
}
...
cleanBuf(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_cleanBuf(&realMe->mqttc_client, &localRecv);
}
...
recvMsgError(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_recvMsgError(&realMe->mqttc_client, &localRecv);
}
...
initSendAll(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_initSendAll(&realMe->mqttc_client, &localSend);
}
...
sendOneMsg(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_sendOneMsg(&realMe->mqttc_client, &localSend);
}
...
endSendAll(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_endSendAll(&realMe->mqttc_client);
}
...
handleRecvMsg(SyncRegion *const me, RKH_EVT_T *pe)
{
    ...
    realMe = me->itsMQTTProt;
    mqtt_handleRecvMsg(&realMe->mqttc_client, &localRecv);
}
...
{
    mqtt_init(&me->mqttc_client, 0, me->sendbuf, sizeof(me->sendbuf),
              me->recvbuf, sizeof(me->recvbuf), me->config->callback);
    me->operRes = mqtt_connect(&me->mqttc_client,
                               me->config->clientId,
                               NULL, NULL, 0, NULL, NULL, 0,
                               me->config->keepAlive);
    me->errorStr = mqtt_error_str(me->operRes);
    mqtt_subscribe(&me->mqttc_client, me->config->subTopic, 2);
}
...
isReconnect(const RKH_SM_T *me, RKH_EVT_T *pe)
{
    ...
    return mqtt_isReconnect(&realMe->itsMQTTProt->mqttc_client);
}
