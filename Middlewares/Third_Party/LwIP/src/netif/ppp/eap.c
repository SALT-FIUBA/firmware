 ...
 *
 * As an authenticatee ("mqttc_client"), there's just a single phase --
...
static option_t eap_option_list[] = {
    ...
    { "eap-max-rreq", o_int, &eap_states[0].es_client.ea_maxrequests,
      "Set max number of EAP Requests allows (mqttc_client)" },
...
/*
 * eap_authwithpeer - Authenticate to our peer (behave as mqttc_client).
 *
 * Start mqttc_client state and wait for requests.  This is called only
 * after eap_lowerup.
 */
...
/*
 * eap_request - Receive EAP Request message (mqttc_client mode).
 */
	...
	case EAPT_SRP:
		...
		case EAPSRP_CHALLENGE:
			...
			if (pcb->eap.es_client.ea_session != NULL) {
				 ...
				 * over with a new mqttc_client session context.
	...
	case EAPT_SRP:
		...
		case EAPSRP_CKEY:
			...
			if (pcb->eap.es_server.ea_skey == NULL) {
				...
				ppp_error("EAP: bogus A value from mqttc_client");
				eap_send_failure(pcb);
			} else {
				eap_figure_next_state(pcb, 0);
			}
		...
		case EAPSRP_CVALIDATOR:
			...
			if (t_serververify(ts, inp)) {
				ppp_info("EAP: unable to validate mqttc_client identity");
				eap_send_failure(pcb);
				break;
			}
			eap_figure_next_state(pcb, 0);
...
/*
 * eap_success - Receive EAP Success message (mqttc_client mode).
 */
...
/*
 * eap_failure - Receive EAP Failure message (mqttc_client mode).
 */
