...
mqtt_initSendAll(struct mqtt_client *client, LocalSendAll *local)
{
    if (client->error < 0 && client->error != MQTT_ERROR_SEND_BUFFER_IS_FULL) {
        ...
        local->len = mqtt_mq_length(&client->mq);
...
mqtt_handleRecvMsg(struct mqtt_client *client, LocalRecvAll *local)
{
    ...
    switch (local->response.fixed_header.control_type) {
        case MQTT_CONTROL_CONNACK:
            /* release associated CONNECT */
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_CONNECT, NULL);
            if (local->msg == NULL) {
        ...
        case MQTT_CONTROL_PUBLISH:
            ...
            if (local->response.decoded.publish.qos_level == 1) {
                local->rv = __mqtt_puback(client, local->response.decoded.publish.packet_id);
                if (local->rv != MQTT_OK) {
                    client->error = local->rv;
                    MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                ...
                if (mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &local->response.decoded.publish.packet_id) != NULL) {
                    break;
                }

                local->rv = __mqtt_pubrec(client, local->response.decoded.publish.packet_id);
                if (local->rv != MQTT_OK) {
                    client->error = local->rv;
                    MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                    local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBACK:
            /* release associated PUBLISH */
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &local->response.decoded.puback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_PUBREC:
            /* check if this is a duplicate */
            if (mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &local->response.decoded.pubrec.packet_id) != NULL) {
                break;
            }
            /* release associated PUBLISH */
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &local->response.decoded.pubrec.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
            ...
            client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (MQTT_PAL_TIME() - local->msg->time_sent);
            /* stage PUBREL */
            local->rv = __mqtt_pubrel(client, local->response.decoded.pubrec.packet_id);
            if (local->rv != MQTT_OK) {
                client->error = local->rv;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBREL:
            /* release associated PUBREC */
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &local->response.decoded.pubrel.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
            ...
            client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (MQTT_PAL_TIME() - local->msg->time_sent);
            /* stage PUBCOMP */
            local->rv = __mqtt_pubcomp(client, local->response.decoded.pubrec.packet_id);
            if (local->rv != MQTT_OK) {
                client->error = local->rv;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBCOMP:
            ...
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &local->response.decoded.pubcomp.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_SUBACK:
            ...
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_SUBSCRIBE, &local->response.decoded.suback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_UNSUBACK:
            ...
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_UNSUBSCRIBE, &local->response.decoded.unsuback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_PINGRESP:
            ...
            local->msg = mqtt_mq_find(&client->mq, MQTT_CONTROL_PINGREQ, NULL);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
