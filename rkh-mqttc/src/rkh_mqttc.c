 ...
 * @note The only files that are included are mqtt.h and rkh_mqttc_pal.h.
 *
 * @cond Doxygen_Suppress
 */

enum MQTTErrors rkh_mqttc_sync(struct rkh_mqttc_client *client) {
    enum MQTTErrors err = __rkh_mqttc_recv(client);
    if (err != MQTT_OK) return err;
    err = __rkh_mqttc_send(client);
    return err;
}
...
uint16_t __rkh_mqttc_next_pid(struct rkh_mqttc_client *client) {
    ...
    do {
        ...
        pid_exists = 0;
        struct rkh_mqttc_queued_message *curr;
        for(curr = rkh_mqttc_mq_get(&(client->mq), 0); curr >= client->mq.queue_tail; --curr) {
...
enum MQTTErrors rkh_mqttc_init(struct rkh_mqttc_client *client,
                          int sockfd,
                          uint8_t *sendbuf, size_t sendbufsz,
                          uint8_t *recvbuf, size_t recvbufsz,
                          void (*publish_response_callback)(void** state,struct rkh_mqttc_response_publish *publish))
{
    ...
    /* initialize mutex */
    RKH_MQTTC_PAL_MUTEX_INIT(&client->mutex);
    RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);

    client->socketfd = sockfd;

    rkh_mqttc_mq_init(&client->mq, sendbuf, sendbufsz);
...
static void __rkh_mqttc_clean_connection(struct rkh_mqttc_client *client)
{
    struct rkh_mqttc_queued_message *msg;
...
#define MQTT_CLIENT_TRY_PACK(tmp, msg, client, pack_call, release)  \
    if (client->error < 0) {                                        \
        if (release) RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);         \
        return client->error;                                       \
    }                                                               \
    tmp = pack_call;                                                \
    if (tmp < 0) {                                                  \
    ...
    msg = rkh_mqttc_mq_register(&client->mq, tmp);                       \


enum MQTTErrors rkh_mqttc_connect(struct rkh_mqttc_client *client,
                             ...
                             uint16_t keep_alive)
{
    ...
    __rkh_mqttc_clean_connection(client);

    /* try to pack the message */
    MQTT_CLIENT_TRY_PACK(rv, msg, client,
                         rkh_mqttc_pack_connection_request(
...
enum MQTTErrors rkh_mqttc_publish(struct rkh_mqttc_client *client,
...
{
    RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);
    uint16_t packet_id = __rkh_mqttc_next_pid(client);
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
    ...
    MQTT_CLIENT_TRY_PACK(
            rv, msg, client,
            rkh_mqttc_pack_publish_request(
                    client->mq.curr, client->mq.curr_sz,
                    topic_name,
                    packet_id,
                    application_message,
...
ssize_t __rkh_mqttc_puback(struct rkh_mqttc_client *client, uint16_t packet_id) {
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
ssize_t __rkh_mqttc_pubrec(struct rkh_mqttc_client *client, uint16_t packet_id) {
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
ssize_t __rkh_mqttc_pubrel(struct rkh_mqttc_client *client, uint16_t packet_id) {
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
ssize_t __rkh_mqttc_pubcomp(struct rkh_mqttc_client *client, uint16_t packet_id) {
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
enum MQTTErrors rkh_mqttc_subscribe(struct rkh_mqttc_client *client,
                               const char* topic_name,
                               int max_qos_level)
        {
                RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);
        ...
        MQTT_CLIENT_TRY_PACK(
                rv, msg, client,
                rkh_mqttc_pack_subscribe_request(
                        client->mq.curr, client->mq.curr_sz,
                        packet_id,
                        topic_name,
                        max_qos_level,
...
enum MQTTErrors rkh_mqttc_unsubscribe(struct rkh_mqttc_client *client,
                                 const char* topic_name)
        {
                RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);
        uint16_t packet_id = __rkh_mqttc_next_pid(client);
        ...
        MQTT_CLIENT_TRY_PACK(
                rv, msg, client,
                rkh_mqttc_pack_unsubscribe_request(
                        client->mq.curr, client->mq.curr_sz,
                        packet_id,
                        topic_name,
                        NULL
...
enum MQTTErrors rkh_mqttc_ping(struct rkh_mqttc_client *client) {
    enum MQTTErrors rv;
    RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);
    rv = __rkh_mqttc_ping(client);
    RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
    return rv;
}

enum MQTTErrors __rkh_mqttc_ping(struct rkh_mqttc_client *client)
{
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
enum MQTTErrors rkh_mqttc_disconnect(struct rkh_mqttc_client *client)
{
    RKH_MQTTC_PAL_MUTEX_LOCK(&client->mutex);
    ssize_t rv;
    struct rkh_mqttc_queued_message *msg;
...
#if 1
void
rkh_mqttc_initSendAll(struct rkh_mqttc_client *client, LocalSendAll *local)
{
    if (client->error < 0 && client->error != MQTT_ERROR_SEND_BUFFER_IS_FULL) {
        ...
        local->len = rkh_mqttc_mq_length(&client->mq);
...
int
rkh_mqttc_isThereMsg(LocalSendAll *local)
{
    return (local->i < local->len);
}

void
rkh_mqttc_sendOneMsg(struct rkh_mqttc_client *client, LocalSendAll *local)
{
    local->msg = rkh_mqttc_mq_get(&client->mq, local->i);
    local->resend = 0;
    if (local->msg->state == MQTT_QUEUED_UNSENT) {
        ...
        /* check for timeout */
        if (RKH_MQTTC_PAL_TIME() > local->msg->time_sent + client->response_timeout) {
            local->resend = 1;
...
void
rkh_mqttc_sendMsgFail(struct rkh_mqttc_client *client, LocalSendAll *local)
{
...
void
rkh_mqttc_setMsgState(struct rkh_mqttc_client *client, LocalSendAll *local)
{
...
int
rkh_mqttc_isInitOk(LocalSendAll *local)
{
    return local->initResult == MQTT_OK;
}

int
rkh_mqttc_isSetMsgStateResult(LocalSendAll *local)
{
    return local->setMsgStateResult == MQTT_OK;
}

void
rkh_mqttc_nextSend(LocalSendAll *local)
{
    ++local->i;
}

int
rkh_mqttc_endSendAll(struct rkh_mqttc_client *client)
{
...
ssize_t __rkh_mqttc_send(struct rkh_mqttc_client *client)
{
    ...
    rkh_mqttc_initSendAll(client, &local);
    if (rkh_mqttc_isInitOk(&local)) {
        /* loop through all messages in the queue */
        for (; rkh_mqttc_isThereMsg(&local); ) {
            rkh_mqttc_sendOneMsg(client, &local);
            if (local.resend) { /* goto next message if we don't need to send */
                if (local.tmp < 0) { /* On receive evSendFail event */
                    rkh_mqttc_sendMsgFail(client, &local);
                    return local.tmp;
                }
                rkh_mqttc_setMsgState(client, &local);
                if (!rkh_mqttc_isSetMsgStateResult(&local)) {
                    RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                    return local.setMsgStateResult;
                }
            }
            rkh_mqttc_nextSend(&local);
        }
        return rkh_mqttc_endSendAll(client);
    }
    RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
...
#if 1
void
rkh_mqttc_initRecvAll(void)
{
}

void
rkh_mqttc_recvAll(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
    local->rv = rkh_mqttc_pal_recvall(client->socketfd, client->recv_buffer.curr,
                                 client->recv_buffer.curr_sz, 0);
}

void
rkh_mqttc_recvFail(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
...
void
rkh_mqttc_parseRecv(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
    if (local->rv != 0)
    {
        ...
        /* attempt to parse */
        local->consumed = rkh_mqttc_unpack_response(&local->response,
...
int
rkh_mqttc_isConsumed(LocalRecvAll *local)
{
    return local->consumed > 0;
}

int
rkh_mqttc_isUnpackError(LocalRecvAll *local)
{
    return local->consumed < 0;
}

void
rkh_mqttc_parseError(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
...
int
rkh_mqttc_isRecvBufFull(LocalRecvAll *local)
{
    return local->noConsumedResult != MQTT_OK;
}

int
rkh_mqttc_noConsumed(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
    (void *)local;
...
int
rkh_mqttc_isNotError(LocalRecvAll *local)
{
    return local->handleRecvMsgResult == MQTT_OK;
}

void
rkh_mqttc_handleRecvMsg(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
    ...
    switch (local->response.fixed_header.control_type) {
        case MQTT_CONTROL_CONNACK:
            /* release associated CONNECT */
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_CONNECT, NULL);
            if (local->msg == NULL) {
            ...
            client->typical_response_time = (double) (RKH_MQTTC_PAL_TIME() - local->msg->time_sent);
        ...
        case MQTT_CONTROL_PUBLISH:
            ...
            if (local->response.decoded.publish.qos_level == 1) {
                local->rv = __rkh_mqttc_puback(client, local->response.decoded.publish.packet_id);
                if (local->rv != MQTT_OK) {
                    client->error = local->rv;
                    RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                ...
                if (rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &local->response.decoded.publish.packet_id) != NULL) {
                    break;
                }

                local->rv = __rkh_mqttc_pubrec(client, local->response.decoded.publish.packet_id);
                if (local->rv != MQTT_OK) {
                    client->error = local->rv;
                    RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                    local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBACK:
            /* release associated PUBLISH */
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &local->response.decoded.puback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
            ...
            client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (RKH_MQTTC_PAL_TIME() - local->msg->time_sent);
            break;
        case MQTT_CONTROL_PUBREC:
            /* check if this is a duplicate */
            if (rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &local->response.decoded.pubrec.packet_id) != NULL) {
                break;
            }
            /* release associated PUBLISH */
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &local->response.decoded.pubrec.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
            ...
            client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (RKH_MQTTC_PAL_TIME() - local->msg->time_sent);
            /* stage PUBREL */
            local->rv = __rkh_mqttc_pubrel(client, local->response.decoded.pubrec.packet_id);
            if (local->rv != MQTT_OK) {
                client->error = local->rv;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBREL:
            /* release associated PUBREC */
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &local->response.decoded.pubrel.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
            ...
            client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (RKH_MQTTC_PAL_TIME() - local->msg->time_sent);
            /* stage PUBCOMP */
            local->rv = __rkh_mqttc_pubcomp(client, local->response.decoded.pubrec.packet_id);
            if (local->rv != MQTT_OK) {
                client->error = local->rv;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = local->rv;
        ...
        case MQTT_CONTROL_PUBCOMP:
            ...
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &local->response.decoded.pubcomp.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_SUBACK:
            ...
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_SUBSCRIBE, &local->response.decoded.suback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_UNSUBACK:
            ...
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_UNSUBSCRIBE, &local->response.decoded.unsuback.packet_id);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
        ...
        case MQTT_CONTROL_PINGRESP:
            ...
            local->msg = rkh_mqttc_mq_find(&client->mq, MQTT_CONTROL_PINGREQ, NULL);
            if (local->msg == NULL) {
                client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                RKH_MQTTC_PAL_MUTEX_UNLOCK(&client->mutex);
                local->handleRecvMsgResult = MQTT_ERROR_ACK_OF_UNKNOWN;
...
void
rkh_mqttc_cleanBuf(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
...
void
rkh_mqttc_recvMsgError(struct rkh_mqttc_client *client, LocalRecvAll *local)
{
    (void *)local;
...
int
rkh_mqttc_isReconnect(struct rkh_mqttc_client *client)
{
    int result = 0;
...
ssize_t __rkh_mqttc_recv(struct rkh_mqttc_client *client)
{
    ...
    rkh_mqttc_initRecvAll();

    while(1) { /* read until there is nothing left to read */
        rkh_mqttc_recvAll(client, &local); /* read in as many bytes as possible */
        if (local.rv < 0) { /* On receive evRecvFail event */
            rkh_mqttc_recvFail(client, &local); /* an error occurred */
            return local.rv;
        }

        rkh_mqttc_parseRecv(client, &local);

        if (rkh_mqttc_isConsumed(&local)) {
            rkh_mqttc_handleRecvMsg(client, &local);
            if (rkh_mqttc_isNotError(&local)) {
                rkh_mqttc_cleanBuf(client, &local);
            }
            else {
                rkh_mqttc_recvMsgError(client, &local);
                return local.handleRecvMsgResult;
            }
        }
        else if (rkh_mqttc_isUnpackError(&local)) {
            rkh_mqttc_parseError(client, &local);
            return local.consumed;
        } else {
            rkh_mqttc_noConsumed(client, &local);
            if (rkh_mqttc_isRecvBufFull(&local)) {
...
ssize_t rkh_mqttc_fixed_header_rule_violation(const struct rkh_mqttc_fixed_header *fixed_header) {
    ...
    required_flags = rkh_mqttc_fixed_header_rules.required_flags[control_type];
    mask_required_flags = rkh_mqttc_fixed_header_rules.mask_required_flags[control_type];

    /* check for valid type */
    if (!rkh_mqttc_fixed_header_rules.control_type_is_valid[control_type]) {
...
ssize_t rkh_mqttc_unpack_fixed_header(struct rkh_mqttc_response *response, const uint8_t *buf, size_t bufsz) {
    struct rkh_mqttc_fixed_header *fixed_header;
    ...
    errcode = rkh_mqttc_fixed_header_rule_violation(fixed_header);
    if (errcode) {
        return errcode;
    }
...
ssize_t rkh_mqttc_pack_fixed_header(uint8_t *buf, size_t bufsz, const struct rkh_mqttc_fixed_header *fixed_header) {
    const uint8_t *start = buf;
    ssize_t errcode;
...
/* CONNECT */
ssize_t rkh_mqttc_pack_connection_request(uint8_t* buf, size_t bufsz,
                                     const char* client_id,
                                     const char* will_topic,
                                     const void* will_message,
                                     size_t will_message_size,
                                     ...
                                     uint16_t keep_alive)
{
    ...
    if (client_id == NULL) {
        ...
        /* rkh_mqttc_string length is strlen + 2 */
        remaining_length += __rkh_mqttc_packed_cstrlen(client_id);
    }

    if (will_topic != NULL) {
        /* there is a will */
        connect_flags |= MQTT_CONNECT_WILL_FLAG;
        remaining_length += __rkh_mqttc_packed_cstrlen(will_topic);
    ...
    if (user_name != NULL) {
        ...
        remaining_length += __rkh_mqttc_packed_cstrlen(user_name);
    } else {
        connect_flags &= ~MQTT_CONNECT_USER_NAME;
    }
    ...
    if (password != NULL) {
        ...
        remaining_length += __rkh_mqttc_packed_cstrlen(password);
    } else {
        connect_flags &= ~MQTT_CONNECT_PASSWORD;
    }
    ...
    rv = rkh_mqttc_pack_fixed_header(buf, bufsz, &fixed_header);
    if (rv <= 0) {
    ...
    *buf++ = connect_flags;
    *(uint16_t*) buf = (uint16_t) RKH_MQTTC_PAL_HTONS(keep_alive);
    buf += 2;

    /* pack the payload */
    buf += __rkh_mqttc_pack_str(buf, client_id);
    if (connect_flags & MQTT_CONNECT_WILL_FLAG) {
        buf += __rkh_mqttc_pack_str(buf, will_topic);
        memcpy(buf, will_message, will_message_size);
        buf += will_message_size;
    }
    if (connect_flags & MQTT_CONNECT_USER_NAME) {
        buf += __rkh_mqttc_pack_str(buf, user_name);
    }
    if (connect_flags & MQTT_CONNECT_PASSWORD) {
        buf += __rkh_mqttc_pack_str(buf, password);
    }
...
/* CONNACK */
ssize_t rkh_mqttc_unpack_connack_response(struct rkh_mqttc_response *rkh_mqttc_response, const uint8_t *buf) {
    const uint8_t *start = buf;
    struct rkh_mqttc_response_connack *response;

    /* check that remaining length is 2 */
    if (rkh_mqttc_response->fixed_header.remaining_length != 2) {
        return MQTT_ERROR_MALFORMED_RESPONSE;
    }

    response = &(rkh_mqttc_response->decoded.connack);
    /* unpack */
    if (*buf & 0xFE) {
...
/* DISCONNECT */
ssize_t rkh_mqttc_pack_disconnect(uint8_t *buf, size_t bufsz) {
    struct rkh_mqttc_fixed_header fixed_header;
    fixed_header.control_type = MQTT_CONTROL_DISCONNECT;
    fixed_header.control_flags = 0;
    fixed_header.remaining_length = 0;
    return rkh_mqttc_pack_fixed_header(buf, bufsz, &fixed_header);
}

/* PING */
ssize_t rkh_mqttc_pack_ping_request(uint8_t *buf, size_t bufsz) {
    struct rkh_mqttc_fixed_header fixed_header;
...
/* PUBLISH */
ssize_t rkh_mqttc_pack_publish_request(uint8_t *buf, size_t bufsz,
                                  const char* topic_name,
                                  uint16_t packet_id,
                                  void* application_message,
                                  size_t application_message_size,
                                  uint8_t publish_flags)
{
    ...
    remaining_length = (uint16_t)__rkh_mqttc_packed_cstrlen(topic_name);
    if (inspected_qos > 0) {
        remaining_length += 2;
    }
    ...
    buf += __rkh_mqttc_pack_str(buf, topic_name);
    if (inspected_qos > 0) {
        *(uint16_t*) buf = (uint16_t) RKH_MQTTC_PAL_HTONS(packet_id);
        buf += 2;
    }
...
ssize_t rkh_mqttc_unpack_publish_response(struct rkh_mqttc_response *rkh_mqttc_response, const uint8_t *buf)
{
    const uint8_t *start = buf;
    struct rkh_mqttc_fixed_header *fixed_header;
    struct rkh_mqttc_response_publish *response;

    fixed_header = &(rkh_mqttc_response->fixed_header);
    response = &(rkh_mqttc_response->decoded.publish);
    ...
    if (rkh_mqttc_response->fixed_header.remaining_length < 4) {
        return MQTT_ERROR_MALFORMED_RESPONSE;
    }

    /* parse variable header */
    response->topic_name_size = (uint16_t) RKH_MQTTC_PAL_NTOHS(*(uint16_t*) buf);
    buf += 2;
    response->topic_name = buf;
    buf += response->topic_name_size;

    if (response->qos_level > 0) {
        response->packet_id = (uint16_t) RKH_MQTTC_PAL_NTOHS(*(uint16_t*) buf);
        buf += 2;
    }
...
/* PUBXXX */
ssize_t rkh_mqttc_pack_pubxxx_request(uint8_t *buf, size_t bufsz,
...
ssize_t rkh_mqttc_unpack_pubxxx_response(struct rkh_mqttc_response *rkh_mqttc_response, const uint8_t *buf)
{
    const uint8_t *start = buf;
    uint16_t packet_id;
    ...
    /* parse packet_id */
    packet_id = (uint16_t) RKH_MQTTC_PAL_NTOHS(*(uint16_t*) buf);
    buf += 2;

    if (rkh_mqttc_response->fixed_header.control_type == MQTT_CONTROL_PUBACK) {
        rkh_mqttc_response->decoded.puback.packet_id = packet_id;
    } else if (rkh_mqttc_response->fixed_header.control_type == MQTT_CONTROL_PUBREC) {
        rkh_mqttc_response->decoded.pubrec.packet_id = packet_id;
    } else if (rkh_mqttc_response->fixed_header.control_type == MQTT_CONTROL_PUBREL) {
        rkh_mqttc_response->decoded.pubrel.packet_id = packet_id;
    } else {
        rkh_mqttc_response->decoded.pubcomp.packet_id = packet_id;
    }
...
/* SUBACK */
ssize_t rkh_mqttc_unpack_suback_response (struct rkh_mqttc_response *rkh_mqttc_response, const uint8_t *buf) {
    const uint8_t *start = buf;
    uint32_t remaining_length = rkh_mqttc_response->fixed_header.remaining_length;
    ...
    /* unpack packet_id */
    rkh_mqttc_response->decoded.suback.packet_id = (uint16_t) RKH_MQTTC_PAL_NTOHS(*(uint16_t*) buf);
    buf += 2;
    remaining_length -= 2;

    /* unpack return codes */
    rkh_mqttc_response->decoded.suback.num_return_codes = (size_t) remaining_length;
    rkh_mqttc_response->decoded.suback.return_codes = buf;
...
/* SUBSCRIBE */
ssize_t rkh_mqttc_pack_subscribe_request(uint8_t *buf, size_t bufsz, uint16_t packet_id, ...) {
    va_list args;
    const uint8_t *start = buf;
    ssize_t rv;
    struct rkh_mqttc_fixed_header fixed_header;
    ...
    for(i = 0; i < num_subs; ++i) {
        ...
        fixed_header.remaining_length += __rkh_mqttc_packed_cstrlen(topic[i]) + 1;
    }
    ...
    *(uint16_t*) buf = (uint16_t) RKH_MQTTC_PAL_HTONS(packet_id);
    ...
    /* pack payload */
    for(i = 0; i < num_subs; ++i) {
        buf += __rkh_mqttc_pack_str(buf, topic[i]);
        *buf++ = max_qos[i];
    }
...
/* UNSUBACK */
ssize_t rkh_mqttc_unpack_unsuback_response(struct rkh_mqttc_response *rkh_mqttc_response, const uint8_t *buf)
{
    const uint8_t *start = buf;
    ...
    /* parse packet_id */
    rkh_mqttc_response->decoded.unsuback.packet_id = (uint16_t) RKH_MQTTC_PAL_NTOHS(*(uint16_t*) buf);
    buf += 2;
...
/* UNSUBSCRIBE */
ssize_t rkh_mqttc_pack_unsubscribe_request(uint8_t *buf, size_t bufsz, uint16_t packet_id, ...) {
    va_list args;
    const uint8_t *start = buf;
    ssize_t rv;
    struct rkh_mqttc_fixed_header fixed_header;
    ...
    for(i = 0; i < num_subs; ++i) {
        ...
        fixed_header.remaining_length += __rkh_mqttc_packed_cstrlen(topic[i]);
    }
...
/* MESSAGE QUEUE */
void rkh_mqttc_mq_init(struct rkh_mqttc_message_queue *mq, void *buf, size_t bufsz)
{
    mq->mem_start = buf;
    mq->mem_end = (char *)buf + bufsz;
    mq->curr = buf;
    mq->queue_tail = mq->mem_end;
    mq->curr_sz = rkh_mqttc_mq_currsz(mq);
}

struct rkh_mqttc_queued_message* rkh_mqttc_mq_register(struct rkh_mqttc_message_queue *mq, size_t nbytes)
{
...
void rkh_mqttc_mq_clean(struct rkh_mqttc_message_queue *mq) {
    struct rkh_mqttc_queued_message *new_head;

    for(new_head = rkh_mqttc_mq_get(mq, 0); new_head >= mq->queue_tail; --new_head) {
    ...
    if (new_head < mq->queue_tail) {
        ...
        mq->curr_sz = rkh_mqttc_mq_currsz(mq);
        return;
    } else if (new_head == rkh_mqttc_mq_get(mq, 0)) {
        /* do nothing */
        return;
    }
    ...
    memmove(rkh_mqttc_mq_get(mq, new_tail_idx), mq->queue_tail, sizeof(struct rkh_mqttc_queued_message) * (new_tail_idx + 1));
    mq->queue_tail = rkh_mqttc_mq_get(mq, new_tail_idx);

    /* bump back start's */
    for(ssize_t i = 0; i < new_tail_idx + 1; ++i) {
        rkh_mqttc_mq_get(mq, i)->start -= removing;
    }
...
struct rkh_mqttc_queued_message* rkh_mqttc_mq_find(struct rkh_mqttc_message_queue *mq, enum MQTTControlPacketType control_type, uint16_t *packet_id)
{
    struct rkh_mqttc_queued_message *curr;
    for(curr = rkh_mqttc_mq_get(mq, 0); curr >= mq->queue_tail; --curr) {
...
/* RESPONSE UNPACKING */
ssize_t rkh_mqttc_unpack_response(struct rkh_mqttc_response* response, const uint8_t *buf, size_t bufsz) {
    const uint8_t *start = buf;
    ssize_t rv = rkh_mqttc_unpack_fixed_header(response, buf, bufsz);
    if (rv <= 0) return rv;
    else buf += rv;
    switch(response->fixed_header.control_type) {
        case MQTT_CONTROL_CONNACK:
            rv = rkh_mqttc_unpack_connack_response(response, buf);
            break;
        case MQTT_CONTROL_PUBLISH:
            rv = rkh_mqttc_unpack_publish_response(response, buf);
            break;
        case MQTT_CONTROL_PUBACK:
            rv = rkh_mqttc_unpack_pubxxx_response(response, buf);
            break;
        case MQTT_CONTROL_PUBREC:
            rv = rkh_mqttc_unpack_pubxxx_response(response, buf);
            break;
        ...
        case MQTT_CONTROL_PUBCOMP:
            ...
            break;
        case MQTT_CONTROL_SUBACK:
            rv = rkh_mqttc_unpack_suback_response(response, buf);
            break;
        case MQTT_CONTROL_UNSUBACK:
            rv = rkh_mqttc_unpack_unsuback_response(response, buf);
            break;
        case MQTT_CONTROL_PINGRESP:
            return rv;
        default:
...
/* EXTRA DETAILS */
ssize_t __rkh_mqttc_pack_str(uint8_t *buf, const char* str) {
    uint16_t length = (uint16_t)strlen(str);

    /* pack string length */
    *(uint16_t*) buf = (uint16_t) RKH_MQTTC_PAL_HTONS(length);
    buf += 2;
...
const char* rkh_mqttc_error_str(enum MQTTErrors error) {
