...
/** 
 * @brief An enumeration of error codes. Error messages can be retrieved by calling \ref rkh_mqttc_error_str.
 * @ingroup api
 * 
 * @see rkh_mqttc_error_str
 */
enum MQTTErrors {
...
#define __rkh_mqttc_packed_cstrlen(x) (2 + strlen(x))
 ...
 * </a>
 */
struct rkh_mqttc_response_publish {
    /** 
 ...
 *
 */
struct rkh_mqttc_response_puback {
 ...
 *
 */
struct rkh_mqttc_response_pubrec {
    /** @brief The published messages packet ID. */
    uint16_t packet_id;
};
 ...
 *
 */
struct rkh_mqttc_response_pubrel {
    /** @brief The published messages packet ID. */
    uint16_t packet_id;
};
 ...
 *
 */
struct rkh_mqttc_response_pubcomp {
 ...
 * </a> 
 */
struct rkh_mqttc_response_suback {
    /** @brief The published messages packet ID. */
    uint16_t packet_id;
 ...
 * </a> 
 */
struct rkh_mqttc_response_unsuback {
    /** @brief The published messages packet ID. */
    uint16_t packet_id;
};
 ...
 * </a> 
 */
/*struct rkh_mqttc_response_pingresp {};*/

/**
 * @brief A struct used to deserialize/interpret an incoming packet from the broker.
 * @ingroup unpackers
 */
struct rkh_mqttc_response {
    /** @brief The rkh_mqttc_fixed_header of the deserialized packet. */
     ...
     */
    union {
        struct rkh_mqttc_response_connack  connack;
        struct rkh_mqttc_response_publish  publish;
        struct rkh_mqttc_response_puback   puback;
        struct rkh_mqttc_response_pubrec   pubrec;
        struct rkh_mqttc_response_pubrel   pubrel;
        struct rkh_mqttc_response_pubcomp  pubcomp;
        struct rkh_mqttc_response_suback   suback;
        struct rkh_mqttc_response_unsuback unsuback;
        /*struct rkh_mqttc_response_pingresp pingresp;*/
    } decoded;
};

/**
 * @brief Deserialize the contents of \p buf into an rkh_mqttc_fixed_header object.
 * @ingroup unpackers
 * 
 * @note This function performs complete error checking and a positive return value
 *       means the entire rkh_mqttc_response can be deserialized from \p buf.
 * 
 * @param[out] response the response who's \ref rkh_mqttc_response.fixed_header will be initialized.
 ...
 * @ingroup unpackers
 * 
 * @pre \ref rkh_mqttc_unpack_fixed_header must have returned a positive value and the control packet type
 *      must be \c MQTT_CONTROL_CONNACK.
 * 
 * @param[out] rkh_mqttc_response the rkh_mqttc_response that will be initialized.
 ...
 * 
 * @relates rkh_mqttc_response_connack
 * 
 * @returns The number of bytes that were consumed, or 0 if the buffer does not contain enough 
 *          bytes to parse the packet, or a negative value if there was a protocol violation.
 */
 ...
 * @ingroup unpackers
 * 
 * @pre \ref rkh_mqttc_unpack_fixed_header must have returned a positive value and the rkh_mqttc_response must
 *      have a control type of \c MQTT_CONTROL_PUBLISH.
 * 
 * @param[out] rkh_mqttc_response the response that is initialized from the contents of \p buf.
 * @param[in] buf the buffer with the incoming data.
 * 
 * @relates rkh_mqttc_response_publish
 * 
 * @returns The number of bytes that were consumed, or 0 if the buffer does not contain enough 
 *          bytes to parse the packet, or a negative value if there was a protocol violation.
 */
 ...
 * @param[in] buf the buffer with the incoming data.
 *
 * @relates rkh_mqttc_response_puback rkh_mqttc_response_pubrec rkh_mqttc_response_pubrel mqtt_response_pubcomp
 * 
 * @returns The number of bytes that were consumed, or 0 if the buffer does not contain enough 
 *          bytes to parse the packet, or a negative value if there was a protocol violation.
 */
 ...
 * @param[in] buf the buffer with the incoming data.
 *
 * @relates rkh_mqttc_response_suback
 * 
 * @returns The number of bytes that were consumed, or 0 if the buffer does not contain enough 
 *          bytes to parse the packet, or a negative value if there was a protocol violation.
 */
 ...
 * @param[in] buf the buffer with the incoming data.
 *
 * @relates rkh_mqttc_response_unsuback
 * 
 * @returns The number of bytes that were consumed, or 0 if the buffer does not contain enough 
 *          bytes to parse the packet, or a negative value if there was a protocol violation.
 */
 ...
 * @ingroup unpackers
 * 
 * @param[out] response the rkh_mqttc_response that will be initialize from \p buf.
 * @param[in] buf the incoming data buffer.
 * @param[in] bufsz the number of bytes available in the buffer.
 * 
 * @relates rkh_mqttc_response
 * 
...
/**
* @brief Serialize an rkh_mqttc_fixed_header and write it to \p buf.
* @ingroup packers
*
* @note This function performs complete error checking and a positive return value
 ...
 * @ingroup packers
 * 
 * @see rkh_mqttc_pack_subscribe_request
 */
 ...
 * @code
 * ssize_t n = rkh_mqttc_pack_subscribe_request(buf, bufsz, 1234, "topic_1", 0, "topic_2", 2, NULL);
 * @endcode
 * 
 ...
 *          packet, a negative value if there was a protocol violation.
 */
ssize_t rkh_mqttc_pack_subscribe_request(uint8_t *buf, size_t bufsz,
                                    uint16_t packet_id,
                                    ...); /* null terminated */
 ...
 * @ingroup packers
 * 
 * @see rkh_mqttc_pack_unsubscribe_request
 */
 ...
 * @note The variadic arguments, \p ..., \em must be followed by a \c NULL. For example:
 * @code
 * ssize_t n = rkh_mqttc_pack_unsubscribe_request(buf, bufsz, 4321, "topic_1", "topic_2", NULL);
 * @endcode
 * 
 ...
 *          packet, a negative value if there was a protocol violation.
 */
ssize_t rkh_mqttc_pack_unsubscribe_request(uint8_t *buf, size_t bufsz,
                                      uint16_t packet_id,
                                      ...); /* null terminated */
...
/**
 * @brief A message in a rkh_mqttc_message_queue.
 * @ingroup details
 */
struct rkh_mqttc_queued_message {
     ...
     */
    rkh_mqttc_pal_time_t time_sent;
 ...
 */
struct rkh_mqttc_message_queue {
    /** 
     ...
     *       \c curr. This is because the rkh_mqttc_queued_message structs share the
     *       same memory (and thus, a rkh_mqttc_queued_message must be allocated in
     *       the message queue's memory whenever a new message is registered).  
     */
    size_t curr_sz;

    /**
     * @brief The tail of the array of rkh_mqttc_queued_messages's.
     * 
     * @note This member should not be used manually.
     */
    struct rkh_mqttc_queued_message *queue_tail;
};
 ...
 * 
 * @relates rkh_mqttc_message_queue
 */
 ...
 * 
 * @note This function will step rkh_mqttc_message_queue::curr and update rkh_mqttc_message_queue::curr_sz.
 * @relates rkh_mqttc_message_queue
 * 
 * @returns The newly added struct rkh_mqttc_queued_message.
 */
...
/**
 * @brief Returns the rkh_mqttc_queued_message at \p index.
 * @ingroup details
 * 
 * @param mq_ptr A pointer to the message queue.
 * @param index The index of the message. 
 *
 * @returns The rkh_mqttc_queued_message at \p index.
 */
#define rkh_mqttc_mq_get(mq_ptr, index) (((struct rkh_mqttc_queued_message*) ((mq_ptr)->mem_end)) - 1 - index)

/**
 * @brief Returns the number of messages in the message queue, \p mq_ptr.
 * @ingroup details
 */
#define rkh_mqttc_mq_length(mq_ptr) (((struct rkh_mqttc_queued_message*) ((mq_ptr)->mem_end)) - (mq_ptr)->queue_tail)

/**
 * @brief Used internally to recalculate the \c curr_sz.
 * @ingroup details
 */
#define rkh_mqttc_mq_currsz(mq_ptr) (mq_ptr->curr >= (uint8_t*) ((mq_ptr)->queue_tail - 1)) ? 0 : ((uint8_t*) ((mq_ptr)->queue_tail - 1)) - (mq_ptr)->curr
...
struct rkh_mqttc_client {
    ...
    */
     ...
     * rkh_mqttc_response_publish messages. All the publishes received from the broker will
     * be passed to this function.
     * 
     ...
     */
    rkh_mqttc_pal_mutex_t mutex;

    /** @brief The sending message queue. */
    struct rkh_mqttc_message_queue mq;
};
 ...
 * 
 * @post rkh_mqttc_connect must be called.
 * 
 ...
 * @note A pointer to \ref rkh_mqttc_client.publish_response_callback_state is always passed as the
 *       \c state argument to \p publish_response_callback. Note that the second argument is 
 *       the rkh_mqttc_response_publish that was received from the broker.
 * 
